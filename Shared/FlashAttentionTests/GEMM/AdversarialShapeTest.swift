...
class AdversarialShapeTester {
    // Adding completion handler for UI feedback
    func runTests(completion: @escaping (String) -> Void) {
        var totalTests = 0
        var failedTests = 0
        
        // Run 20 trials instead of original 100 for faster feedback
        for testIndex in 0..<20 {
            ...
            do {
                try runCorrectnessTest(descriptor: gemmDesc)
                totalTests += 1
            } catch {
                failedTests += 1
                completion("Test \(testIndex + 1) failed: \(error.localizedDescription)")
            }
            
            let progress = "Test \(testIndex + 1)/20 completed. Failed: \(failedTests)"
            completion(progress)
        }
        
        let finalResult = "Testing complete. Total: \(totalTests), Failed: \(failedTests)"
        completion(finalResult)
    }
    
    private func randomPrecision() -> GEMMOperandPrecision {
